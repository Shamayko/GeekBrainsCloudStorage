...
public class Controller implements Initializable {
    ...
    public static void sendFile(String tfFileName) throws IOException {
        try {
            FileSender.sendFile(Paths.get("client_storage/" + tfFileName), Network.getInstance().getCurrentChannel(), future -> {
                if (!future.isSuccess()) {
                    JOptionPane.showConfirmDialog(null, "Sending error");
                }
                if (future.isSuccess()) {
                    JOptionPane.showConfirmDialog(null, "Sending successful");
                }
            });
    ...
    public void pressOnDownloud(ActionEvent actionEvent) {
        if (!tfFileName.getText().isEmpty()) {
            ...
            sendFileRequest(filename, Network.getInstance().getCurrentChannel(), future -> {
                if (!future.isSuccess()) {
                    JOptionPane.showConfirmDialog(null, "Downloading error");
                }
                if (future.isSuccess()) {
                    JOptionPane.showConfirmDialog(null, "Downloading successful");
                }
            });
            tfFileName.clear();
        }
    ...
    public void pressOnDelete(ActionEvent actionEvent) {
        if (!tfFileName.getText().isEmpty()) {
            ...
            sendDeleteRequest(filename, Network.getInstance().getCurrentChannel(), future -> {
                if (!future.isSuccess()) {
                    JOptionPane.showConfirmDialog(null, "Deleting error");
                }
                if (future.isSuccess()) {
                    JOptionPane.showConfirmDialog(null, "Deleting successful");
                }
            });
            tfFileName.clear();
        }
    ...
    public void pressOnAuth(ActionEvent actionEvent) throws InterruptedException {
        GBCWindow.newWindowAuth("Authorization");
        Thread.sleep(1000);
    ...
    public void pressOnSignUn(ActionEvent actionEvent) {
        GBCWindow.newWindowSignIn("Registration");
    }
